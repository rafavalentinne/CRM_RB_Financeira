...
(USERNAME, PASSWORD, GET_PHONE, SELECT_BANK,
 SELECT_RESULT, GET_NOTE) = range(6)

(GET_NEW_USER_NAME, GET_NEW_USER_LOGIN, GET_NEW_USER_PASS,
 GET_NEW_USER_ROLE, GET_NEW_USER_SUPERVISOR) = range(6, 11)
...
async def _enviar_info_cliente(update: Update, context: ContextTypes.DEFAULT_TYPE, cliente: dict,
    ...
    texto_principal = (
        ...
        f"<b>CPF:</b> <code>{cliente['cpf']}</code>\n"
        f"<b>Telefone:</b> <code>{cliente['telefone']}</code>"
    )
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton("✍️ Adicionar Nota", callback_data=f"add_note_{cliente['_id']}")
        ],
        [
